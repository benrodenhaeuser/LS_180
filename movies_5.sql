...
-- FROM 1-M to M-M:
-- 1. the films table needs a fresh primary key column
-- 2. create a join table with foreign keys
-- 3. capture the existing 1toM relationships in the join table
-- 4. drop the directors column from the films table
...
-- show a list of films and their directors
...
ORDER BY count(films.id) DESC, directors.name ASC;
